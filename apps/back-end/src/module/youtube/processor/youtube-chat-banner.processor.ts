...
@Processor(YOUTUBE_CHAT_BANNER_QUEUE_NAME, {
  autorun: BooleanUtil.fromString(process.env.PROCESSOR_AUTORUN),
  concurrency: NumberUtil.parse(process.env.YOUTUBE_CHAT_BANNER_QUEUE_CONCURRENCY, 1),
  maxStalledCount: NumberUtil.parse(process.env.QUEUE_MAX_STALLED_COUNT, QUEUE_MAX_STALLED_COUNT),
})
export class YoutubeChatBannerProcessor extends BaseProcessor {
  protected readonly logger = new Logger(YoutubeChatBannerProcessor.name)
